...
Cloud vector database layer, built on Pinecone.

Design decision — per-meeting namespaces:
    Each meeting gets its own Pinecone namespace (namespace = meeting_id).
    This keeps meetings cleanly separated (no accidental cross-meeting
    leakage during a normal per-meeting Q&A), while still letting us do
    deliberate CROSS-meeting search (agent/memory_tools.py) by looping
    over multiple namespaces on purpose.

    Pinecone does not support "search all namespaces in one call" natively,
    so cross-meeting search here queries each namespace individually and
    merges the results by score. Fine for a few dozen meetings; if this
    project ever needs to scale to thousands of meetings, that loop should
    be swapped for a single flat index with meeting_id stored as metadata
    and filtered instead of namespaced.

Before using this file, add to api/config.py (Config class):
...
...and to your .env:
    ...
    PINECONE_INDEX_NAME=meeting-assistant
    PINECONE_CLOUD=aws
...
class VectorStore:
    ...
    def query_cloud(
        ...
        """
        Search stored meeting chunks by semantic similarity.

        - meeting_ids=None  -> searches across ALL stored meetings
          (cross-meeting search, used by agent/memory_tools.py)
        - meeting_ids=[...] -> restricts search to only those meeting(s)

        Returns a list of dicts, sorted by score descending:
            [{"meeting_id": str, "date": str, "text": str, "score": float}, ...]
        """
