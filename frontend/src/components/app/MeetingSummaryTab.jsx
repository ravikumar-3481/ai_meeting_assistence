...
import { 
  FileText, Sparkles, CheckCircle2, Target, TrendingUp, 
...
export default function MeetingSummaryTab({ summary, title, participants }) {
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-6 text-zinc-200">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-br from-zinc-900 via-zinc-900 to-indigo-950/40 border border-indigo-500/30 shadow-xl space-y-4">
      ...
      <div className="p-6 rounded-2xl bg-zinc-900/80 border border-zinc-800 space-y-4">
        ...
        <div className="space-y-3">
          {summary.keyTakeaways.map((item, idx) => (
            <div key={idx} className="p-3.5 rounded-xl bg-zinc-950/80 border border-zinc-800/80 text-xs sm:text-sm text-zinc-300 flex items-start gap-3">
              <span className="w-6 h-6 rounded-full bg-indigo-500/20 text-indigo-400 font-mono text-xs flex items-center justify-center shrink-0">
                {idx + 1}
              </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {participants.map((p, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-between text-xs">
              ...
              <span className="font-mono text-indigo-400 font-medium bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20">
                {p.timeSpoken} spoken
              </span>
            </div>
          ))}
