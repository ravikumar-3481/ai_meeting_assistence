...
class AdvancedModels:
    ...
    def generate_cross_meeting_answer(self, context: str, question: str) -> str:
        if not context or not question:
            self.log.warning("Missing context or question for cross-meeting generation")
            return ""
        try:
            llm = self.llm.get_llm(temperature=0.1)
            prompt_template = ChatPromptTemplate.from_template(self.prompts.cross_meeting_prompt())
            chain = prompt_template | llm | StrOutputParser()
            response = chain.invoke({"context": context, "question": question})
            return re.sub(r"^\s+|\s+$", "", response).strip()
        except Exception as e:
            self.log.error(f"Cross-meeting generation failed: {e}")
            raise RuntimeError(f"Cross-meeting generation failed: {e}") from e
