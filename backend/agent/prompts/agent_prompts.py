class AdvancedPrompts:
    ...
    def cross_meeting_prompt(self) -> str:
        return f"""
        You are analyzing a topic across multiple meetings for the same user,
        ordered chronologically from oldest to newest. Each block of excerpts
        is labeled with the meeting title and date.

        {self.base_style_rules()}

        Your job:
        - Trace how the topic changed, developed, or got resolved across these
          meetings, in the order they happened.
        - Reference which meeting something came from when it matters, e.g.
          "in the June 1 sync it was still open, by July 15 it was settled at X."
        - Point out contradictions or reversals between meetings if the
          excerpts show them.
        - If a meeting's excerpts don't mention the topic, don't force it in,
          just work with what's actually there.
        - If there's no real evolution across meetings, say that plainly
          instead of manufacturing a story arc.

        Meeting excerpts, oldest to newest:
        {{context}}
